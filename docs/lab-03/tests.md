...
Tests are specified before the feature PRs. Each server test uses isolated, deterministic fixture Users/Tickets and removes only rows/files it creates; migration tests run both a fresh schema and an upgraded Lab 2 fixture. Database clock/randomness/throttle storage are injectable for deterministic auth/expiry/race tests. Legacy Lab 1/Lab 2 behavioral assertions remain regression evidence; only identity setup and expectations that are obsolete because of authenticated Lab 3 (the Development Requester selector and `X-Requester-Id`) may be intentionally adapted. Server suites remain sequential while sharing the configured local database, as documented in Lab 2.
...
| MIG-01 | Migration | AC-11, BR-03, BR-10, BR-17, specification §9 | Upgraded Lab 2 fixture retains User IDs, Ticket IDs/numbers/requester links, Attachment metadata/files/uploader/remover links; IT Priority is copied. For a legacy Requester with no credential hash, provision the documented local-only Initial Password as a non-null bcrypt cost-12 hash and set `mustChangePassword = true` without exposing the password/hash; Login with that Initial Password succeeds, a normal protected route is rejected with mandatory-change enforcement, Change Password succeeds, Login with the changed password succeeds, and Login with the Initial Password then fails. | `server/tests/lab-03/migration-seed.regression.test.ts` | Planned |
| MIG-02 | Migration | AC-11, AC-12, BR-03, BR-10, specification §9 | Fresh/upgraded migration and repeat seed: required account distribution/workflow fixtures and no duplicate rows; after MIG-01 changes the legacy Requester credential, repeat migration/seed preserves the User and credential/hash, keeps `mustChangePassword = false`, accepts the changed password, rejects the Initial Password, and emits no password/hash value in responses or logs | `server/tests/lab-03/migration-seed.regression.test.ts` | Planned |
| REG-01 | Regression manifest | AC-04, AC-11, AC-12, BR-29 | Runs exactly the existing Lab 1/Lab 2 server/client suites named in §5 against the migrated schema; only authenticated identity setup and obsolete Development Requester selector/`X-Requester-Id` expectations are intentionally adapted. Migration assertions remain owned by MIG-01/MIG-02, not this ID. | Exact existing paths in §5, `REG-01 legacy manifest` | Planned |
...
| AC-11 | MIG-01, MIG-02, REG-01 | `server/tests/lab-03/migration-seed.regression.test.ts`; REG-01 legacy manifest in §5 |
| AC-12 | E2E-R01, MIG-02, REG-01, EVD-D01, EVD-D02, EVD-D03 | `e2e/lab-03/capture.screens.ts`; `server/tests/lab-03/migration-seed.regression.test.ts`; REG-01 legacy manifest in §5; `docs/lab-03/reviewer.md`; `docs/lab-03/ai-use.md`; `docs/lab-03/tests.md` |
...
| FR-20 | AC-11, AC-12 | MIG-01, MIG-02, REG-01 | `server/tests/lab-03/migration-seed.regression.test.ts`; REG-01 legacy manifest in §5 |
...
| BR-10 | AC-01, AC-02, AC-10, AC-11 | API-A03, UI-H01, E2E-A01, MIG-01, MIG-02 | `server/tests/lab-03/auth.api.test.ts`; `client/tests/lab-03/AppShell.test.tsx`; `e2e/lab-03/authentication.spec.ts`; `server/tests/lab-03/migration-seed.regression.test.ts` |
...
| BR-29 | AC-04, AC-11, AC-12 | API-R01, API-AT01, REG-01 | `server/tests/lab-03/requester-regression.api.test.ts`; `server/tests/lab-03/attachments.api.test.ts`; REG-01 legacy manifest in §5 |
...
`REG-01` owns this exact existing legacy server/client regression manifest; it does not claim migration assertions (MIG-01/MIG-02 own those) or Lab 2 E2E/screenshot capture:
