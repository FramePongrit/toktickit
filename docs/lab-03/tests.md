...
| API-R01 | API | AC-04, BR-17, BR-29 | Authenticated Requester creates every Requested Priority; each created Ticket has matching `itPriority`, supplied `itPriority` is rejected, and later read regression preserves immutable Requested Priority | `server/tests/lab-03/requester-regression.api.test.ts` | Planned |
...
| API-C03 | API | AC-05, BR-24 | Requester Resolution Indication records once, changes no status, and rejects duplicate/Resolved/final eligibility cases | `server/tests/lab-03/comments-notes.api.test.ts` | Planned |
...
| API-S02 | API | AC-07, AC-08, BR-18-BR-20, BR-24, BR-25, BR-31 | Concurrent Claim has exactly one success/one 409; concurrent Reassign using one `expectedOwnerId` has exactly one success and one `409 TICKET_OWNER_CHANGED` with safe current owner; stale Reassign never overwrites; Final/Unassigned precede malformed target, target probing remains identical `OWNER_NOT_ELIGIBLE`, and Claim against an owned Ticket remains `TICKET_ALREADY_ASSIGNED` | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Planned |
| API-S03 | API | AC-07, AC-08, BR-24, BR-31 | IT Priority update is independent/Role-protected and never changes Requested Priority; Final plus invalid IT Priority returns `409 TICKET_FINAL`, while only a Non-final invalid enum returns `400 VALIDATION_FAILED` | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Planned |
...
| REG-01 | Regression | AC-04, AC-11, AC-12, BR-29 | All Lab 1/Lab 2 server and client behavioral assertions run against migrated schema; only authenticated identity setup and obsolete Development Requester selector/`X-Requester-Id` expectations are intentionally adapted | `server/tests/lab-03/migration-seed.regression.test.ts` | Planned |
...
| AC-08 | API-S02, API-S03, API-S04, UI-S01, E2E-S01 | `server/tests/lab-03/staff-ticket-detail.api.test.ts`; `client/tests/lab-03/StaffTicketDetail.test.tsx`; `e2e/lab-03/staff-ticket-flow.spec.ts` |
...
| FR-06 | AC-04 | API-Z02, API-C01, API-R01, API-AT01, REG-01, UI-R01, E2E-A01, E2E-S01 | `server/tests/lab-03/authorization.api.test.ts`; `server/tests/lab-03/comments-notes.api.test.ts`; `server/tests/lab-03/requester-regression.api.test.ts`; `server/tests/lab-03/attachments.api.test.ts`; `server/tests/lab-03/migration-seed.regression.test.ts`; `client/tests/lab-03/RequesterTicketDetail.test.tsx`; `e2e/lab-03/authentication.spec.ts`; `e2e/lab-03/staff-ticket-flow.spec.ts` |
...
| BR-17 | AC-04, AC-07 | API-R01, API-S03, UI-S01, E2E-S01 | `server/tests/lab-03/requester-regression.api.test.ts`; `server/tests/lab-03/staff-ticket-detail.api.test.ts`; `client/tests/lab-03/StaffTicketDetail.test.tsx`; `e2e/lab-03/staff-ticket-flow.spec.ts` |
...
| BR-29 | AC-04, AC-11, AC-12 | API-C01, API-R01, API-AT01, REG-01 | `server/tests/lab-03/comments-notes.api.test.ts`; `server/tests/lab-03/requester-regression.api.test.ts`; `server/tests/lab-03/attachments.api.test.ts`; `server/tests/lab-03/migration-seed.regression.test.ts` |
