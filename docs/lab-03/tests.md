...
| API-S02 | API | AC-07, AC-08, BR-18-BR-20, BR-24, BR-31 | Concurrent Claim has exactly one success/one 409; Final Ticket guard wins over already-assigned/invalid Owner target; Reassign of an Unassigned Ticket returns `409 TICKET_UNASSIGNED` and requires Claim before payload/target checks; only then validate malformed, missing, inactive, or wrong-Role Owner targets | `server/tests/lab-03/staff-ticket-detail.api.test.ts` | Planned |
...
| E2E-S01 | E2E | AC-04, AC-05, AC-06, AC-07, AC-08, AC-10, BR-30, BR-31 | Requester/staff flow: safe eligible-Owner directory drives Queue filter and Reassign, queue Search/filter/sort/page, Claim/Reassign, IT Priority, status edges/Waiting comment, Public Comments and Internal Notes on `RESOLVED`, Attachment continuity, indication, and Final-over-invalid priority/status/comment/note feedback | `e2e/lab-03/staff-ticket-flow.spec.ts` | Planned |
...
| BR-31 | AC-03, AC-05, AC-07, AC-08 | API-Z01, API-C02, API-C04, API-S02, API-S03, API-S04, UI-S01, UI-S02, E2E-S01 | `server/tests/lab-03/authorization.api.test.ts`; `server/tests/lab-03/comments-notes.api.test.ts`; `server/tests/lab-03/staff-ticket-detail.api.test.ts`; `client/tests/lab-03/StaffTicketDetail.test.tsx`; `e2e/lab-03/staff-ticket-flow.spec.ts` |
