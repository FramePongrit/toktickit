...
Page header: **Ticket Queue**, active/all scope segmented control, and one-line scope description. Filter bar: labelled Search (Ticket Number, Summary, Requester name/email), Status, IT Priority, Category, Owner (All, My Tickets, Unassigned, eligible user), Sort, Order, page size, and Clear Filters. Changes reset page to 1 and preserve query state in the URL (`scope`, `q`, `status`, `itPriority`, `categoryId`, `owner`, `sort`, `order`, `page`, `pageSize`) for reproducible navigation. Debounce Search; filters combine visibly as AND.
